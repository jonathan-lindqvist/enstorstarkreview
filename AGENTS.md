...
**Map.** `/karta` uses MapLibre GL JS with OpenFreeMap's keyless Liberty style. It always opens centered on Göteborg at zoom 12.5; do not automatically fit or recenter it to markers. The server-only `server/review-map.ts` loads only `PUBLIC_REVIEW_FILTER` reviews, matches normalized addresses against persisted `map_geocodes`, and serializes each marker with only the fields the client needs. Resolved coordinates are retained permanently; `not_found` results retry after 30 days and temporary failures after one hour. The 24-hour in-process marker-list cache rebuilds from MongoDB on expiry—it must never trigger a bulk Nominatim refresh. After the map becomes interactive, a signed-in editor can POST to `/karta/next-marker`, which resolves at most one uncached public address; anonymous visitors may view cached markers but must never trigger geocoding. Nominatim lookups are process-wide single-flight and at least one second apart; a successful lookup invalidates the map cache. The endpoint requires `locals.user` and checks the request origin. Draft creation or draft edits must not invalidate this cache. `ReviewMap.svelte` must import `maplibre-gl/dist/maplibre-gl-worker.mjs?url` and call `setWorkerUrl` before constructing the map; direct MapLibre worker loading fails under Vite development. Keep MapLibre CSS bundled locally and retain the OpenFreeMap and worker CSP allowances in `svelte.config.js`. Do not restore a resolved/total map counter: an invalid or unmatched address may legitimately never receive a marker.
