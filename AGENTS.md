...
Node-native scripts (require Node 18+ and a `.env` with `MONGO_URI`; `engine-strict` is on):
...
**Request lifecycle.** `src/hooks.server.ts` starts the Mongo connection once, validates the Lucia session cookie on every request into `event.locals.user` / `event.locals.session`, and sets security headers (nosniff, referrer-policy, `X-Frame-Options: DENY`, permissions-policy). The CSP lives separately in `svelte.config.js` (nonce mode, allowlists Google Analytics). SvelteKit `+page.server.ts` `load`/`actions` are the only entry points that touch the DB.

**Database.** `src/lib/db/db.ts` holds the singleton `MongoClient` and the `enstorstark` database; it **throws at import if `MONGO_URI` is unset**. `start_mongo()` (called from hooks) creates all indexes: the unique `bars.slug` index and TTL indexes for `audit_logs` (90d) and `login_rate_limits` (30d). Collections are thin typed wrappers (`bars.ts`, `users.ts`). `db/init.js` seeds demo users and the slug index for the **dev** container only.
...
Prettier: tabs, single quotes, no trailing commas, `printWidth` 100, LF. Follow existing patterns — server logic belongs in `src/lib/server/*` (unit-tested in isolation), routes stay thin orchestration.
